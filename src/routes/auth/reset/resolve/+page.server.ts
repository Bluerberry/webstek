...
import { UserService, SessionService, PasswordResetService } from '$server/services'
...
export const actions: Actions = {
	resolve: async ({ url, request, locals, cookies }) => {
		...
		// Get password reset
		const passwordReset = await PasswordResetService.getByUserId(user.id, false)
		if (!passwordReset) {
		...
		// Resolve
		const resolved = await PasswordResetService.resolve(
			passwordReset.id,
